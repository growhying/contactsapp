<!-- App.vue로부터 contactlist 데이터 속성을 props로 전달받아 화면을 구성한다.-->
<template>
  <div>
    <p class="addnew">
...
<script>
  ...
  export default {
    ...
    props: [ 'contactlist' ],
    methods: {
      // 컴포넌트 내부에는 데이터가 없으므로 부모 컴포넌트의 데이터를 변경하도록 하기 위해
      // 이벤트 버스 객체를 통해 $emit을 호출하고, App.vue에서 $on으로 이벤트를 수신하여 처리한다.
      addContact : function() {
...
<style scoped>
  .addnew {
    margin: 10px auto; max-width: 820px; min-width: 820px; padding: 40px 0 0 0; text-align: left;
  }
  #example {
    margin: 10px auto; max-width: 820px; min-widows:820px; padding: 0; position: relative; font: 13px "verdana";
  }
  #example .long {
    width: 100%;
  }
  ...
  #list th:nth-child(5n+4), #list td:nth-child(5n+4) { width: 150px; }
  #list th {
  ...
  img.thumbnail {
    width: 48px; height: 48px; margin-top: auto; margin-bottom: auto; display: block; cursor: pointer;
  }
